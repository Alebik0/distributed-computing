...
class Context(object):
    ...
    def send(self, msg: Message, to: str):
        ...
        """
        if len(msg.type) > 50:
            raise ValueError('message type length exceeds the limit of 50 '
                             'characters')
        if not isinstance(to, str):
            raise TypeError('to argument has to be string, not {}'.format(
                            type(to)))
    ...
    def set_timer(self, timer_name: str, delay: float):
        """
        Sets a timer that will trigger on_timer callback after the
        specified delay. If there is an active timer with this name,
        its delay is overridden.
        """
        if not isinstance(timer_name, str):
            raise TypeError('timer_name argument has to be str, not {}'.format(
                            type(timer_name)))
        if len(timer_name) > 50:
            raise ValueError('timer_name length exceeds the limit of 50 '
                             'characters')
        if not isinstance(delay, (int, float)):
            raise TypeError('delay argument has to be int or float, '
                            'not {}'.format(type(delay)))
        if delay < 0:
